...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    if (empty($input_description)) {
        $description_err = "Please enter the description amount.";
    ...
    if (empty($name_err) && empty($tittle_err) && empty($description_err)) {
        ...
        $sql = "UPDATE ticket SET name=?, tittle=?, description=? WHERE id=?";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_tittle, $param_description, $param_id);
